...
public class DefaultGoogleStyleProgressLayout extends GoogleStyleProgressLayout {
	...
	public DefaultGoogleStyleProgressLayout(Context context, TypedArray attrs) {
		super(context, attrs);
		initInflate(context, com.handmark.pulltorefresh.library.R.layout.pull_to_refresh_progress_google_style);
		initProperties();
		reset();
	}
	/**
	...
	private void initProperties() {
		mActionBarHeaderPullingProgressLayout = (PullingProgressLayout) findViewById(com.handmark.pulltorefresh.library.R.id.pulling_progress);
		mActionBarHeaderRefreshingProgressBar = (ProgressBar) findViewById(com.handmark.pulltorefresh.library.R.id.refreshing_progress);
	}
	/**
